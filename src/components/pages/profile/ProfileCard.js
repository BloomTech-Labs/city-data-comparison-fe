      ...
      <div className="profile-contents">
        <div className="avatar-tab">
          <div className="username">
            <p>Username</p>
            <h2 className={`user-name ${nameEdit.status}`}>
              {userInfo.googleid === null && userInfo.linkedinid === null
                ? `${userInfo.username}`
            ...
            <form
              className={`edit-name ${nameEdit.status}`}
              onSubmit={updateUser}
            >
              <input
          ...
          <img
            src={
              !user.userimage
                ? `${ProfileImage}`
                : `https://be.citrics.io/${user.userimage}`
            }
          />
          <form
            className={`edit-image ${imageUpload.status}`}
            action="/uploads"
          ...
          <button
            className={`edit-image-btn ${imageUpload.status}`}
            onClick={toggleImage}
          >
            Upload Image
          </button>
          <button
            className={`save-image-btn ${imageUpload.status}`}
            onClick={() => {
              toggleImage();
              postImage();
            }}
        ...
        <div className=" info">
          ...
          <div className="email-tab">
            <p>Email</p>
            <h2 className={`user-email ${emailEdit.status}`}>
              {userInfo.email}
            </h2>
            <form
              className={`edit-email ${emailEdit.status}`}
              onSubmit={updateUser}
            >
              <input
                onChange={handleChange}
          ...
          <div className="city-tab">
            ...
            <h2 className={`user-location ${locationEdit.status}`}>
            ...
            <button
              className={`edit-location-btn ${locationEdit.status}`}
              onClick={() => {
                toggleLocation();
                toggleEmail();
                toggleName();
            ...
            <button
              className={`save-location-btn ${locationEdit.status}`}
              onClick={() => {
                toggleLocation();
                toggleEmail();
                toggleName();
