    ...
    <div>
      ...
      {selected.map((city) => (
        ...
        >
          <div
            ...
            onMouseOver={(e) =>
              Number.isNaN(parseFloat(e.target.getAttribute("latitude"))) ||
              setPopState({
                ...popState,
                lat: parseFloat(e.target.getAttribute("latitude")),
                lng: parseFloat(e.target.getAttribute("longitude")),
                posleft: e.target.getBoundingClientRect().left,
            ...
            onMouseLeave={(e) =>
              setPopState({ ...popState, display: "none", animate: false })
            }
          >
            <img
              src={activepin}
              alt={`A map pin indicating ${city.name}`}
              latitude={city.Latitude}
              longitude={city.Longitude}
            />
          </div>
      ...
      {cityMarkers.map((cityMarker) => {
        return (
          <Marker
            key={cityMarker.ID}
            latitude={cityMarker.lat}
            longitude={cityMarker.lng}
          >
            ...
            >
              {selected.find((item) => item._id === cityMarker.ID) ? (
                <img
                  ...
                  src={activepin}
                  alt={`A map pin indicating ${cityMarker.city}`}
                  latitude={cityMarker.lat}
                  longitude={cityMarker.lng}
                />
              ) : (
      ...
      <div id="popupmap">
        <PopupMap
          lat={popState.lat}
          lng={popState.lng}
          posleft={popState.posleft}
          postop={popState.postop}
          display={popState.display}
          city={popState.city}
          animate={popState.animate}
        />
      </div>
    </div>
  );
