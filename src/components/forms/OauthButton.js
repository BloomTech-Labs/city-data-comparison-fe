...
const OauthButton = (props) => {
  ...
  return (
    <div
      className={`oauthButton ${props.company}Button`}
      onClick={() => {
        connectAccount();
      }}
    >
      <div className="iconContainer">
        <img
          className={`oauthIcon ${props.company}.icon`}
          src={props.icon}
          alt={`${props.company} icon`}
        />
      </div>
      <p className={`oauthName ${props.company}Name`}>
        {props.action} with {props.company}
      </p>
    </div>
  );
};
